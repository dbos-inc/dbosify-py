"""Common Temporal exceptions, mirroring ``temporalio.exceptions``.

Constructors and properties match the Temporal Python SDK exactly, with one
substitution: where temporalio exposes a protobuf ``Failure`` via the
``failure`` property, we expose the serialized failure envelope (a plain
dict, see ``_internal/payloads.py``) or ``None``.
"""
...
class FailureError(TemporalError):
    ...
    def __init__(
        ...
        *,
        ...
        exc_args: Optional[Tuple[Any, ...]] = None,
    ) -> None:
...
class ActivityAlreadyStartedError(FailureError):
    ...
    def __init__(
        self, activity_id: str, activity_type: str, *, run_id: Optional[str] = None
    ) -> None:
...
def is_cancelled_exception(exception: BaseException) -> bool:
    ...
    return (
        ...
        or (
            isinstance(exception, (ActivityError, ChildWorkflowError))
