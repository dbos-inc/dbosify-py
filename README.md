...
| 17 | Interceptors cover client (`Client(interceptors=)`) and activity (`Worker(interceptors=)`); workflow inbound/outbound interception is Phase 4. There is no header-propagation channel through the workflow, so `headers` on every interceptor `*Input` is inert; worker interceptors come only from `Worker(interceptors=)` (the worker has no client to harvest them from). See [DEVIATIONS.md](DEVIATIONS.md) D24. |
