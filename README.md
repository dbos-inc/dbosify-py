...
| 20 | Worker **deployment versioning** is mirrored and backed by DBOS: a deployment version is `WorkerDeploymentVersion(deployment_name=<DBOS app name>, build_id=<DBOS application_version>)`, overridable via `Worker(build_id=)`/`Worker(deployment_config=)` and readable via `workflow.info().get_current_deployment_version()`/`get_current_build_id()`. The pin/auto-upgrade *behavior* (`VersioningBehavior` on `@workflow.defn`, `ContinueAsNewVersioningBehavior`, `versioning_override`/`versioning_intent`) is accepted-and-inert — DBOS pins dequeue to `application_version` and has no routing fleet. See [DEVIATIONS.md](DEVIATIONS.md) D29. |
