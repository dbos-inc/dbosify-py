"""Namespace -> DBOS schema mapping (ARCHITECTURE no-server)."""
...
def test_default_namespace_is_not_privileged() -> None:
    ...
    assert namespace_schema(DEFAULT_NAMESPACE) == "dbosify_default"
...
@pytest.mark.parametrize(
    ...
    [
        ("alt", "dbosify_alt"),
        ("my_namespace", "dbosify_my_namespace"),
        ("team_a", "dbosify_team_a"),
        ("_internal", "dbosify__internal"),
        ("ns123", "dbosify_ns123"),
        ("a" * 55, "dbosify_" + "a" * 55),  # max length once prefixed (63)
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "a" * 56,  # too long once prefixed (>63)
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "public",
        "dbosify_",  # prefix only, empty namespace
        "dbosify_With-Hyphen",  # invalid namespace suffix
        "myschema",
    ],
)
