"""Message-passing chaos (Phase 2 exit): SIGKILL with message handlers
mid-flight, plus the is_replaying() probe.

The update-staging machinery introduced new recovery windows: an update can
be *accepted* (durable acceptance event, client already unblocked) while its
handler is still parked. Killing there and recovering must replay inbox
delivery order, re-park the handlers, run their effects exactly once, and
still answer both update results — including to a client that reconnects
after the crash (``get_update_handle``).
"""
...
def test_is_replaying_across_recovery(tmp_path: Path) -> None:
    ...
    # [early, mid, late] from the *recovered* execution: early sits below
    # the checkpoint horizon (replaying); mid is evaluated right after the
    # last recorded event was consumed (the live frontier — Temporal also
    # reports False while processing the final, non-replayed task); late is
    # driven by a fresh signal (live).
    ...
    # workflow.logger suppresses during replay: the early log line was
    # emitted live by the first process, replayed-and-suppressed by the
    # second; the late one is live in the second.
...
async def test_sigkill_with_updates_in_flight(tmp_path: Path) -> None:
    ...
    try:
        ...
        # Recovery re-parked both handlers; release them and re-attach to
        # the in-flight updates by id, as a crashed-and-restarted caller
        # would.
    ...
    # Inbox delivery order is checkpointed, so the replayed prefix is exact;
    # the two handlers wake together on release, so their completions may
    # interleave either way (consistently on any given replay).
