...
``dbosify``. Connection setup (the documented remaining migration
...
CACHE_DIR = Path(tempfile.gettempdir()) / "dbosify-conformance" / "samples-python"
...
def rewrite_sample(source: Path, dest_dir: Path) -> Path:
    ...
    file's path.
    """
    rewritten = _IMPORT_ROOT_RE.sub("dbosify", source.read_text())
